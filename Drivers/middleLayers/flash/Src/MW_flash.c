...
flashError_t MW_flashElase(/*!erase here!*/const void *flash_add,size_t size){
  /*消去から。一ページ削除。周辺にコードを置かないようにリンカスクリプトを変更すること。*/
  {
    ...
    if(HAL_FLASHEx_Erase(&elase_hand, &page_err)!=HAL_ERROR){
...
static
flashError_t MW_flashUnlock(void){
  if(HAL_FLASH_Unlock()!=HAL_OK)return MW_FLASH_UNLOCK_FAILURE;
  return MW_FLASH_OK;
}

static
flashError_t MW_flashLock(void){
  if(HAL_FLASH_Lock()!=HAL_OK)return MW_FLASH_LOCK_FAILURE;
  return MW_FLASH_OK;
}
